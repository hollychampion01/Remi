...
mtrn3100::PIDController angleController(3.5, 0.05, 0.0); // Kp, Ki, Kd
...
const float forwardSpeedLeft    = 117.6f;

// Turn motion
const float TURN_DEG            = 90.0f;
const float TURN_MAX_PWM        = 60.5f;
const float TURN_COMPLETE_DEG   = 8.0f;   // slightly looser for robustness
...
bool executeRotation() {
  ...
  float err = targetYaw_unwrapped - yaw_unwrapped; // plain difference in ℝ
  ...
  if (fabsf(err) > TURN_COMPLETE_DEG && !timeout) {
    // Same-sign on both motors = spin-in-place for your wiring
