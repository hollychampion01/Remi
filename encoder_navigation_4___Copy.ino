...
mtrn3100::PIDController leftMotorHeadingController(2.0, 0.1, 0.2); // PID for heading correction during forward movement
mtrn3100::PIDController rightMotorHeadingController(1, 0.05, 0.0); // Right motor PID (unused)
...
const float forwardSpeedRight    = 113.0f;
...
bool executeForward() {
  ...
  if (avg < FORWARD_DISTANCE_MM) { // Continue forward until we reach the target distance
    ...
    // Initialize heading PID on first call
    if (!headingPIDInitialized) {
      leftMotorHeadingController.zeroAndSetTarget(0.0f, 0.0f);  // Target zero error
    ...
    // Use PID on the error (note: sign flipped in compute, so we negate output)
    float pidOutput = leftMotorHeadingController.compute(headingErr);
    float headingAdjustment = -pidOutput;  // Correct sign
    headingAdjustment = constrain(headingAdjustment, -HEADING_MAX_ADJUST, HEADING_MAX_ADJUST);

    // Apply differential steering
    float leftPWM = forwardSpeedLeft - headingAdjustment;
    float rightPWM = forwardSpeedRight + headingAdjustment;

    // NOTE: opposite sign on left motor for forward motion
    motor1.setPWM(rightPWM);  // Right motor
    motor2.setPWM(-leftPWM);  // Left motor (negative for forward)

    // Optional debug print every 200ms
    static unsigned long lastDebug = 0;
    if (millis() - lastDebug > 200) {
      Serial.print("FWD after turn: tgt=");
      Serial.print(forwardHeadingTarget, 1);
      Serial.print(", cur=");
      Serial.print(ahrs_angle, 1);
      Serial.print(", err=");
      Serial.print(headingErr, 1);
      Serial.print(", adj=");
      Serial.print(headingAdjustment, 1);
      Serial.print(", PWM L/R=");
      Serial.print(leftPWM, 0);
      Serial.print("/");
      Serial.println(rightPWM, 0);
      lastDebug = millis();
    }

    return false; // Keep moving forward
  } else {
    headingPIDInitialized = false;  // Reset for next forward command
    ...
    if (commandInProgress) {
      ...
      if (done) {
        ...
        if (cmd == 'l' || cmd == 'r') {
          ...
          // Update the forwardHeadingTarget AFTER the turn is completed
          forwardHeadingTarget = ahrs_angle;
        }
      }
    }
...
void stopRobot() {
  ...
  stopHeadingController.zeroAndSetTarget(ahrs_angle, forwardHeadingTarget - ahrs_angle);
