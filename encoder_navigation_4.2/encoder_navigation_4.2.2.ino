...
mtrn3100::PIDController leftMotorHeadingController(0.05, 0.0, 0.0); // Left motor PID
mtrn3100::PIDController rightMotorHeadingController(0.05, 0.0, 0.0); // Right motor PID
...
// Forward motion
float FORWARD_DISTANCE_MM  = 330.0f;  // Default forward distance (in mm)
const float forwardSpeedLeft     = 120.0f;
const float forwardSpeedRight    = 123.0f;

// Turn motion
float TURN_DEG = 45.0f;             // Default turn angle (in degrees)
...
// Command string (example) - this can be modified or updated dynamically
String comm = "f330r45f170l90f500r30f250l45f200"; // A sample set of commands
...
bool executeRotation() {
  ...
  if (fabsf(err) > TURN_COMPLETE_DEG && !timeout) {
    ...
    motor1.setPWM(desiredSign * mag);
    motor2.setPWM(desiredSign * mag);

    return false; // Keep turning
  } else {
    motor1.setPWM(0);
    motor2.setPWM(0);
    started = false;
...
void loop() {
  if (currentCommandIndex < comm.length()) {
    ...
    if (!commandInProgress) {
      if (cmd == 'f') {
        // Forward movement with distance parsing
        encoder1Start = encoder1.getDistanceMM();
        encoder2Start = encoder2.getDistanceMM();
        updateIMU();
        forwardHeadingTarget = ahrs_angle;

        // Parse distance (next digits)
        currentCommandIndex++;  // Move to the next character after 'f'
        String distanceStr = "";
        while (currentCommandIndex < comm.length() && isDigit(comm.charAt(currentCommandIndex))) {
          distanceStr += comm.charAt(currentCommandIndex);
          currentCommandIndex++;
        }
        // If a distance was found, update FORWARD_DISTANCE_MM
        if (distanceStr.length() > 0) {
          FORWARD_DISTANCE_MM = distanceStr.toFloat();
        }

        commandInProgress = true;
        
      } else if (cmd == 'l' || cmd == 'r') {
        // Turn movement with angle parsing
        (void)safeRecalibrateGyroBiasForTurn();
        updateIMU();

        // Parse angle (next digits)
        currentCommandIndex++;  // Move to the next character after 'l' or 'r'
        String angleStr = "";
        while (currentCommandIndex < comm.length() && isDigit(comm.charAt(currentCommandIndex))) {
          angleStr += comm.charAt(currentCommandIndex);
          currentCommandIndex++;
        }
        // If an angle was found, update TURN_DEG
        if (angleStr.length() > 0) {
          TURN_DEG = angleStr.toFloat();
        }

        // Update the target yaw based on the turn direction
        float delta = (cmd == 'l') ? +TURN_DEG : -TURN_DEG;
        targetYaw_unwrapped = yaw_unwrapped + delta;
        angleController.zeroAndSetTarget(yaw_unwrapped, targetYaw_unwrapped);
        commandInProgress = true;

        // After turning, update forwardHeadingTarget to the new heading
        forwardHeadingTarget = ahrs_angle;  // Reset heading to the new heading after turn
      } else {
        currentCommandIndex++;  // Move to the next character if no command
      }
    }
